...
GCP Bucket
Local: files live on the host and are mounted into the container
...
In BigQuery, count can be answered using metadata (so the estimate may be 0 bytes or very small), because it doesn’t necessarily need to scan.
In ClickHouse, a similar optimization (metadat read)